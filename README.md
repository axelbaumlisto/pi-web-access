...
- **EXTERNAL — `web_search`**: the live internet across OpenAI/Codex, Exa, Brave, Parallel, Tavily, SERPdive, AnySearch, self-hosted SearXNG, Perplexity, Gemini (+ URL fetch, GitHub clone, PDF, YouTube/video understanding, Firecrawl extraction, source_check claim verification).
...
| **Sources** | OpenAI/Codex, Exa, Brave, Parallel, Tavily, Perplexity, Gemini, SearXNG, DuckDuckGo, Firecrawl, Jina, TinyFish, Search1API, Searchinfinity, Querit, SERPdive, Kagi, Ollama, Bocha, AnySearch, XCrawl, Valyu, xAI, Mistral, Bright Data, SerpBase, Serper, Kimi | pi sessions (JSONL), claude-recall DB, `*.md` docs, git log/diffs |
...
When several providers are fronted by the SAME API gateway (e.g. a self-hosted proxy that injects a pooled key), one base host + one key is enough:

```json
{
  "proxyBaseUrl": "https://your-gateway.example",
  "proxyApiKey": "sk-proxy-..."
}
```

Or via env: `WEB_SEARCH_PROXY_URL` / `WEB_SEARCH_PROXY_KEY`. Fronted providers resolve to the gateway route; per-provider `*BaseUrl` / `*_BASE_URL` overrides still win; providers the gateway does not front keep their defaults.

| Provider | Gateway route | Kind |
|---|---|---|
| exa | `${proxyBaseUrl}/v1/exa` (+ `/search`, `/answer`, `/mcp` by the provider) | base |
| brave | `${proxyBaseUrl}/v1/brave/search` | full |
| perplexity | `${proxyBaseUrl}/v1/chat/completions` | full |
| openai | `${proxyBaseUrl}/v1/responses` | full |
| tavily, parallel | not fronted — vendor default or their own `*BaseUrl` | base |
| gemini | separate: `geminiBaseUrl` + `geminiApiKey` (own resolver: Cloudflare AI Gateway / ADC detection) | — |

**Not the same thing as `proxy`.** Upstream's `"proxy"` config / tool parameter is a **transport** HTTP(S) proxy (every request is tunnelled through `curl`). `proxyBaseUrl` is an **API gateway** — *which host answers*, not *how packets travel*. They are orthogonal; both may be set.

**Base vs full URLs.** Per-provider overrides follow upstream semantics and are **bases**: `braveBaseUrl` is `…/res/v1` and the provider appends `/web/search`; `tavilyBaseUrl` gets `/search`; `exaBaseUrl` gets `/search` / `/answer`. (Until 1.1.0 brave/tavily were full URLs — see CHANGELOG.) Gateway routes are complete as listed above. All configured URLs — per-provider and `proxyBaseUrl` — are validated: absolute HTTPS, no embedded credentials, no query/fragment.

**Destination-first key binding.** The credential is chosen AFTER the destination resolves:
